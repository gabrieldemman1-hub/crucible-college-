...
export ADMIN_NAME=Ana
export FIRM_NAME="Example Law Group"
export AGENT_NAME=Maya
...
- Storage is set to "everything" so the Retell dashboard shows transcripts during the demo. Production turns audio off; see `docs/retell-setup.md` item 11.
- Whisper: the briefing is generated by the model from the conversation, following `prompts/briefing.md`. Hanging up during the whisper counts as declining; Maya then reads the fallback message to the caller.
