...
- Transfers (default `TRANSFER_MODE=agentic`): right before transferring, Maya saves the caller's details (`save_caller_details`). A separate staff-side agent (one each for intake and admin, ids in `.retell-ids.json`) briefs the staff member from those details, answers questions, and connects the caller only on a clear yes. No, silence, voicemail, or call screening sends the caller back to Maya for the callback message. `TRANSFER_MODE=warm` restores the old whisper, where anything but a question connects.
- Callback alerts: `poc/alerts/callback-alerts.gs` is a Google Apps Script that emails when a caller needs a callback. Deploy it as a web app (steps at the top of the file), then run `create-agent.mjs` with `ALERT_WEBHOOK_URL` set to its URL plus `?key=`.
